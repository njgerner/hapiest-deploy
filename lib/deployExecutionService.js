...
class DeployExecutionService {
     ...
     */
    constructor(s3, eb, info, folders, logger, preHookFunction) {
        this._s3 = s3;
        this._eb = eb;
        this._info = info;
     ...
     *
     *      1) Check that deployment settings are ok (e.g., a commit hash has been provided)
     *      2) Run pre-hook function if provided (DeployService passes it or not based on input flag)
     *      3) If pre-hook function successful then run deploy!
     *
     * @param {string} [commitHash] - will use if provided in place of looking up commit hash from repository
     * @returns {Promise}
     */
    deploy(commitHash) {
        return Promise.resolve()
            .then(() => this._confirmSettingsOk(commitHash))
            .then(() => this._executePreHookFunction(commitHash))
            .then(() => this._deployToEb(commitHash))
    }

    _confirmSettingsOk(commitHash) {
        if (!commitHash && !this._info.gitBranch) {
    ...
    _executePreHookFunction(commitHash) {
        if (this._preHookFunction) {
            return Promise.resolve()
                .tap(() => this._logger.info('Deploy: executing pre-hook function'))
                .then(() => this._preHookFunction(this._info, commitHash, this._logger))
        } else {
            return Promise.resolve()
                .tap(() => this._logger.info('Deploy: not executing pre-hook function'));
        }
    }
     ...
     * @returns {Promise}
     */
    _deployToEb(commitHash) {
        return Promise.resolve()
            .tap(() => this._logger.info('Deploy: attempting to deploy application', {info:this._info, commitHash:commitHash}))
            .then(() => {
                return this._getCommitHash(commitHash)
                    .then(_commitHash => {
     ...
     * @private
     */
    _getCommitHash(commitHash) {
        const commitHashPromise = commitHash ? Promise.resolve(commitHash) : this._getCommitHashFromRepo();
