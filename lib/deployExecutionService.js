...
class DeployExecutionService {
    ...
    deploy() {
        return Promise.resolve()
            ...
            .then(() => {
                let sha;
                return this._getCommitSha()
                .then(localSha => {
                    sha = localSha;
                    return this._createEbApplicationBundleBuffer(sha);
                })
                .then(appBundleBuffer => this._uploadEbApplicationBundleToS3(sha, appBundleBuffer))
            })
     ...
     * @private
     */
    _createEbApplicationBundleBuffer(sha) {
        const zip = new AdmZip();
        const localPath = Path.join(this._folders.apps, this._info.appName,'Dockerrun.aws.json');
        const dockerrunContents = Fs.readFileSync(localPath, 'utf-8').replace('{{TAG}}',`:${sha}`);
        zip.addFile('Dockerrun.aws.json', dockerrunContents);
        return zip.toBuffer();
    }
