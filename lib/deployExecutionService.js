...
class DeployExecutionService {
    ...
    _createEbApplicationBundleBuffer(commitHash) {
        ...
        const ebExentensionsContents = this._fetchEbExtensionsFolderContents();
        zip.addFile('Dockerrun.aws.json', new Buffer(dockerrunContents));
        zip.addFolder('.ebextensions', new Buffer(ebExentensionsContents));
        return zip.toBuffer();
    }
     ...
     * @private
     */
    _fetchEbExtensionsFolderContents() {
        ...
        return Fs.readdirSync(localEbExtensionsFolderPath);
    }
